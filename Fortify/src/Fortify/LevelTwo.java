...
public class LevelTwo extends BasicGameState {
	...
	static int xCleric = 7;
	static int yCleric = 6;
...
//===================================================================================================================
	...
	public void init(GameContainer container, StateBasedGame game) throws SlickException {
		...
		try{
			map = new TiledMap("Fortify/resource/levelTwo.tmx");
...
//===================================================================================================================
	...
	public void render(GameContainer container, StateBasedGame game, Graphics g) throws SlickException {
		...
		g.drawImage(ResourceManager.getImage(Game.GUIDING_BOLT_ICON_RSC).getScaledCopy((float) 2), 1150, 200);
		g.drawImage(ResourceManager.getImage(Game.HEAL_ICON_RSC).getScaledCopy((float) 2), 1000, 350);
		g.drawImage(ResourceManager.getImage(Game.MASS_HEAL_ICON_RSC).getScaledCopy((float) 2), 1150, 350);
...
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&-------------------------------------------------------------&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
		...
		if( xTile < 29 && yTile < 25){
			...
			} else if(currentDragged == 2){
				
				if(posX <= 928 && tileSet[xTile][yTile].getPlayerType() > 0 && posY <= 800){
					g.drawImage(ResourceManager.getImage(Game.GUIDING_BOLT_ICON_RSC), posX-20, posY-20, Color.green);
				} else {
					g.drawImage(ResourceManager.getImage(Game.GUIDING_BOLT_ICON_RSC), posX-20, posY-20);
				}
			...
			} else if(currentDragged == 4){
				
				if(posX <= 928 && tileSet[xTile][yTile].getPlayerType() > 0 && posY <= 800){
					g.drawOval(posX-100, posY-100, 200, 200);
					g.drawImage(ResourceManager.getImage(Game.MASS_HEAL_ICON_RSC), posX-20, posY-20, Color.green);
				} else {
					g.drawOval(posX-100, posY-100, 200, 200);
					g.drawImage(ResourceManager.getImage(Game.MASS_HEAL_ICON_RSC), posX-20, posY-20);
				}
	...
	public void update(GameContainer container, StateBasedGame game, int delta) throws SlickException {
		...
		if(input.isMouseButtonDown(Input.MOUSE_LEFT_BUTTON)){
			...
			if(currentDragged == 0) {
				if(posX <= 1100 && posX >= 1000 && posY >= 200 && posY <= 285){
					currentDragged = 1;
				} else if(posX <= 1250 && posX >= 1150 && posY >= 200 && posY <= 290){
					currentDragged = 2;
				} else if(posX <= 1100 && posX >= 1000 && posY >= 350 && posY <= 435){
					currentDragged = 3;
				} else if(posX <= 1250 && posX >= 1150 && posY >= 350 && posY <= 440){
					currentDragged = 4;
		...
		//------------------------------------------------------------
			...
			if(posX <= 928 && currentDragged != 0){
				...
				} else if(currentDragged == 2 && bolt > 0){
					//bolt goes here
					if(posX <= 928 && posY <= 800){
						boltX = xTile;
						boltY = yTile;
					}
				...
				} else if(currentDragged == 4 && massHeal > 0){
		...
		if(countdown < 0 && monsterQuantity > 0){
			...
			int mType = rm.nextInt(2)+1;
			monsters.add(new EnemyCharacters(mType, (monsterX * 32), (monsterY * 32)+5));
			countdown = 11000;
			monsterQuantity--;
		}
		...
		if(massH == true) {
			for (int i = 0; i < 25; i++) {
				for (int j = 0; j < 29; j++) {
					if(tileSet[j][i].pc != null){
						int distX = j - massHX; 
						int distY = i - massHY;
						double distHeal = Math.sqrt(distX*distX + distY*distY);
						
						if(distHeal <= 3 && massH == true){
							tileSet[j][i].setHeal();
						}
					}
				}
		...
		for(Iterator<Node> n = dijkstraGraph.iterator(); n.hasNext();){
			...
			for(Iterator<EnemyCharacters> e = monsters.iterator(); e.hasNext();){
				...
				if(myX == boltX && myY == boltY){
					enemy.characterHit(12);
					bolt--;
				}
				...
				if(tileSet[cycleNode.x][cycleNode.y].getCollision() == 1 && tileSet[cycleNode.x][cycleNode.y].pc != null){
					int distX = cycleNode.x - massHX; 
					int distY = cycleNode.y - massHY;
					double distHeal = Math.sqrt(distX*distX + distY*distY);
					
					if(distHeal <= 3 && massH == true){
						tileSet[cycleNode.x][cycleNode.y].setHeal();
					}

					distX = cycleNode.x - myX;
					distY = cycleNode.y - myY;
...
//===================================================================================================================
	...
	public void newUnits(){
		for(int i = 0; i < 2; i++){
			...
			if(unit <= 25){
				...
				//wall
				wall++;
			} else if (unit > 50 && unit <= 65){
				//shield
				shield++;
			} else if (unit > 65 && unit <= 75){
				//heal
				heal++;
			} else if (unit > 75 && unit <= 85){
				//archer
				archer++;
				archerCard = true;
				showCardCountdown = 1500;
			} else if(unit > 85 && unit <= 90){
				//mass heal
				massHeal++;
			} else if(unit > 95 && unit <= 100){
				//guiding bolt
				bolt++;
			}
		}	
	...
	public static void initVars(){
		...
		projectiles = new ArrayList<Projectile>();
				...
				tileSet[6][4].setWall(1);tileSet[7][4].setWall(1);tileSet[8][4].setWall(1);
				tileSet[6][8].setWall(1);tileSet[7][8].setWall(1);tileSet[8][8].setWall(1);
				
				tileSet[5][5].setWall(2);tileSet[5][6].setWall(2);tileSet[5][7].setWall(2);
				
				tileSet[9][5].setWall(3);tileSet[9][6].setWall(3);tileSet[9][7].setWall(3);
